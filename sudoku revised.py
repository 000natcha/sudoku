...
class Timer:
    ...
    def checkhighscore(self):
        gameminutes, gameseconds = self.get_elapsed_minutes_seconds()
        #print(gameminutes, gameseconds)
        if gameminutes <= Timer.highscore[0]:
            if gameseconds <= Timer.highscore[1]:
                Timer.highscore[0] = gameminutes
                Timer.highscore[1] = gameseconds
                Timer.highscore[2] = 1
            else:
                pass
...
fonthighscore = pygame.font.SysFont(None, 60) #for timer
timer = Timer()
firstrun = True
...
pass_sound = pygame.mixer.Sound('sounds/win.mp3')
fail_sound = pygame.mixer.Sound('sounds/fail.mp3')
button_sound = pygame.mixer.Sound('sounds/button.mp3')

pass_screen = pygame.image.load('screens/pass.png')
fail_screen = pygame.image.load('screens/fail.png')
...
def background():
    ...
    if Timer.highscore[2] == 0:
        highscore_text = fonthighscore.render("HS 00:00",True, BLACK)
    else:
        highscore_text = fonthighscore.render("HS {:02d}:{:02d}".format(Timer.highscore[0], Timer.highscore[1]), True, BLACK)
    window.blit(timer_text, (865, 255))
    window.blit(highscore_text, (910, 385))
...
while True:
    for event in pygame.event.get():
      ...
      if endButton.handleEvent(event):
        ...
        timer.stop()
         #timer.checkhighscore() leave it here to check if highscore works
    ...
    if answers == 0: #answers not submitted
       pass
    elif answers == 1: #correct
       window.blit(pass_screen, (125,41))
    elif answers == 2: #incorrect
       window.blit(fail_screen, (125,41))
